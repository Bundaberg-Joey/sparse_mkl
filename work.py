...
if __name__ == '__main__':
    ...
    X, y = make_regression(n_samples=100, n_features=5, n_informative=3, noise=1.0, random_state=1)
    X_ind = X[-10:]
        
    sparse_model = SparseGaussianProcessGPy(X_inducing=X)
    sparse_model.fit(X_train=X, y_train=y)
    mu, std = sparse_model.predict(X, return_std=True)
    print(mu)
    print(y)
